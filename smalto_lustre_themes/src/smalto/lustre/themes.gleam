...
pub fn a11y_dark() -> Config(msg) {
  ...
  |> smalto_lustre.keyword(
    styled_span([#("color", "highlight"), #("font-weight", "bold")]),
  )
  |> smalto_lustre.string(styled_span([#("color", "highlight")]))
  |> smalto_lustre.number(styled_span([#("color", "#00e0e0")]))
  |> smalto_lustre.comment(styled_span([#("color", "highlight")]))
  |> smalto_lustre.function(
    styled_span([#("color", "highlight"), #("font-weight", "bold")]),
  )
  |> smalto_lustre.operator(
    styled_span([#("color", "highlight"), #("font-weight", "bold")]),
  )
  ...
  |> smalto_lustre.selector(
    styled_span([#("color", "#abe338"), #("font-weight", "bold")]),
  )
  |> smalto_lustre.property(styled_span([#("color", "highlight")]))
  |> smalto_lustre.regex(styled_span([#("color", "#ffd700")]))
  |> smalto_lustre.custom(custom_renderer(
    important_styles: [#("color", "window")],
