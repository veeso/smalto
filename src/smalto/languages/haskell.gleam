...
pub fn grammar() -> Grammar {
  Grammar(name: "haskell", extends: option.None, rules: rules())
}
...
fn rules() -> List(Rule) {
  [
    ...
    grammar.rule(
      "number",
      "(?i)\\b(?:\\d+(?:\\.\\d+)?(?:e[+-]?\\d+)?|0o[0-7]+|0x[0-9a-f]+)\\b",
    ),
    grammar.greedy_rule("operator", "`(?:[A-Z][\\w']*\\.)*[_a-z][\\w']*`"),
    grammar.rule("operator", "(?<=\\s)\\.(?=\\s)"),
    grammar.rule(
      "operator",
      "[-!#$%*+=?&@|~:<>^\\\\\\/][-!#$%*+=?&@|~.:<>^\\\\\\/]*|\\.[-!#$%*+=?&@|~.:<>^\\\\\\/]+",
    ),
    ...
    grammar.rule_with_inside("constant", "\\b(?:[A-Z][\\w']*\\.)*[A-Z][\\w']*", [
      grammar.rule("punctuation", "\\."),
    ]),
    grammar.rule("punctuation", "[{}[\\];(),.:]"),
  ]
