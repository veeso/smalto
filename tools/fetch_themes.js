...
/**
 * Fetches all Prism.js themes and adapts them for Smalto's hl- class prefix.
 *
...
const BUNDLED_BASE =
  "https://raw.githubusercontent.com/PrismJS/prism/master/themes";
...
const COMMUNITY_BASE =
  "https://raw.githubusercontent.com/PrismJS/prism-themes/master/themes";
...
function convertCss(css, filename) {
  ...
  // :not(pre) > code[class*="language-"] → .smalto-inline
  result = result.replace(
    /:not\(pre\)\s*>\s*code\[class\*=["']language-["']\]/g,
    ".smalto-inline"
  );

  // pre[class*="language-"] > code → .smalto code
  result = result.replace(
    /pre\[class\*=["']language-["']\]\s*>\s*code/g,
    ".smalto code"
  );
  ...
  result = result.replace(
    /div\.code-toolbar\s*>\s*\.toolbar/g,
    ".smalto > .toolbar"
  );
...
async function main() {
  ...
  for (let i = 0; i < allThemes.length; i += 8) {
    const batch = allThemes.slice(i, i + 8);
    const results = await Promise.allSettled(
      batch.map(async ({ file, base }) => {
        ...
        const outName = file.replace(/^prism-?/, "smalto-").replace(/^smalto-\.css$/, "smalto-default.css");
