...
function parseCssTheme(cssContent) {
  ...
  // Strip @media blocks before parsing — their nested braces confuse the flat
  // regex, and high-contrast / print overrides should not leak into token styles.
  const stripped = cssContent.replace(/@media[^{]*\{([\s\S]*?\})\s*\}/g, '');

  // Extract base text color from `.smalto, .smalto code { color: ... }`
  const baseMatch = stripped.match(/\.smalto\s*,\s*\.smalto\s+code\s*\{[^}]*color:\s*([^;]+);/);
  if (baseMatch) {
 ...
 */
function extractDescription(cssContent, filename) {
  ...
  if (comments && comments.length >= 2) {
    ...
    // Skip separator blocks that are just asterisks/whitespace (e.g. /******/)
    if (cleaned && !/^\*+\s*\w*$/.test(cleaned) && !/^\*{3,}/.test(cleaned)) {
      return cleaned;
    }
  }

  // Fallback: derive a human-readable name from the CSS filename
